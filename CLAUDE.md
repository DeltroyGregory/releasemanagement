...
**Data layer:** EF Core Code First against SQL Server. Local dev points at `.\SQLEXPRESS` (see `appsettings.Development.json`); the deployed `dev` environment uses Azure SQL with Managed Identity auth instead (see `.devops/`). `AppDbContext` extends `IdentityDbContext<ApplicationUser>`; see [Data/AppDbContext.cs](Data/AppDbContext.cs) for the full `DbSet` list. Two migrations exist so far (`InitialCreate`, `AddSecurityProfile` — see [Migrations/](Migrations/)).
...
**Routing** ([ClientApp/src/app/app.routes.ts](ClientApp/src/app/app.routes.ts)): `/login` is a standalone top-level route (no guard). Everything else sits under a single `Shell` layout route, gated by `authGuard`. Default route is `dashboard`. Routes: `dashboard`, `my-releases` (releases mine-only), `releases` (all releases), `releases/new`, `releases/:id`, `releases/:id/edit`, `admin/users`, `admin/permissions` (the last two also gated by `adminGuard`).
...
- `core/models.ts` + `core/services/` — one service per entity (`release.ts`, `task.ts`, `release-system.ts`, `app-version.ts`, `fix-version.ts`, `comment.ts`, `auth.ts`, `user.ts`, `permissions.ts`).
